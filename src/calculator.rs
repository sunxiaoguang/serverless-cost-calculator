...
pub struct WorkloadUsage {
    row_based_storage_in_gib: u64,
    network_egress_in_gib: u64,
...
fn calculate(
    ...
    WorkloadEstimation {
        storage_cost: usage.row_based_storage_in_gib as f64 * row_based_price,
        request_units_cost: (usage.network_egress_in_gib + usage.request_units_in_million) as f64
            * ru_price,
        free_credit,
    }
}
...
fn estimate_usage(workload: WorkloadDescription) -> WorkloadUsage {
    ...
    WorkloadUsage {
        row_based_storage_in_gib: (workload.total_data_in_bytes + workload.total_index_in_bytes)
            / GIGA,
        network_egress_in_gib: workload.sent_bytes_per_hour / GIGA,
